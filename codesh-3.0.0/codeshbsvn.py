...
def svn1(command):
    if Options.debuglevel == 0:
        com = command.replace('svn ', 'svn --quiet ', 1)
    else:                                                 
        com = command
        print('svn will run: ' + com)
    os.system(com)
...
def checkRepo(sourceRepo):
   try:
     if (sourceRepo.find("file://")) >= 0 or (sourceRepo.find("svn://")) >= 0:
         fileExists = 1
     else: raise
   except:
...
# SVN back-end
    if Repository.type == 'svn':     # svn
        ...
        if Repository.flag == 1:
            ...
            if Repository.location.find("@") > 0: # Check if the SVN repository is remote and the user can login.
                homeDir =  os.environ.get('HOME')
                [firstPart, secondPart] = Repository.location.split('@')
                [protocol,userName] = firstPart.split('//')
                [host,repoLocation] = secondPart.split('/')
                flgSvnUser = 0
                flgHost = 0
            ...
            while not fileExists: # to check if protocol file:// or svn:// is included for svn in the repository path
                try:
                    if (Repository.location.find("file://")) >= 0 or (Repository.location.find("svn://")) >= 0:
                       fileExists = 1
                    else: raise
                except:
                    print("This is SVN repository")
