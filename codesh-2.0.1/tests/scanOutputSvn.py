...
for k in tempkeys:
    ...
    for line in outputfile.readlines():
        ...
        if (string.find(shortline,testDict[k][0]) != -1):
            success = 1
            if (len(testDict[k]) == 1):
                success = 2
                print "Test-"+k+" SUCCESSFUL: "+`infoDict[k]`
                break
        
        # only when len(testDict[k]) == 2 we can enter this block
        if ((success == 1) and (string.find(shortline,testDict[k][1]) != -1)):
            success = 2
            print "Test-"+k+" SUCCESSFUL: "+`infoDict[k]`
            break
        
    if(success < 2):
        print "Test-"+k+" *==> UNSUCCESSFUL <==*: "+`infoDict[k]`
